...
ALIGN(64) static const uint64_t blake2b_IV[8] = {0x6a09e667f3bcc908ULL,
...
static inline int blake2b_param_set_salt(
    blake2b_param* P,
...
static inline int blake2b_param_set_personal(
    blake2b_param* P,
...
int blake2b_final(blake2b_state* S, uint8_t* out, uint8_t outlen) {
  ...
  memset(
      S->buf + S->buflen, 0, 2 * BLAKE2B_BLOCKBYTES - S->buflen); /* Padding */
  blake2b_compress(S, S->buf);
  memcpy(out, &S->h[0], outlen);
  return 0;
}

int blake2b(uint8_t* out,
            const void* in,
            const void* key,
            const uint8_t outlen,
