...
static inline int blake2bp_init_leaf(blake2b_state* S,
                                     uint8_t outlen,
                                     uint8_t keylen,
                                     uint64_t offset) {
  blake2b_param P[1];
...
static inline int blake2bp_init_root(blake2b_state* S,
                                     uint8_t outlen,
                                     uint8_t keylen) {
  blake2b_param P[1];
  P->digest_length = outlen;
...
int blake2bp_update(blake2bp_state* S, const uint8_t* in, uint64_t inlen) {
  ...
  if (left && inlen >= fill) {
    ...
    for (size_t i = 0; i < PARALLELISM_DEGREE; ++i)
      blake2b_update(
          S->S[i], S->buf + i * BLAKE2B_BLOCKBYTES, BLAKE2B_BLOCKBYTES);
