...
static inline int blake2sp_init_leaf(blake2s_state* S,
                                     uint8_t outlen,
                                     uint8_t keylen,
                                     uint64_t offset) {
  blake2s_param P[1];
...
static inline int blake2sp_init_root(blake2s_state* S,
                                     uint8_t outlen,
                                     uint8_t keylen) {
  blake2s_param P[1];
  P->digest_length = outlen;
...
int blake2sp_update(blake2sp_state* S, const uint8_t* in, uint64_t inlen) {
  ...
  if (left && inlen >= fill) {
    ...
    for (size_t i = 0; i < PARALLELISM_DEGREE; ++i)
      blake2s_update(
          S->S[i], S->buf + i * BLAKE2S_BLOCKBYTES, BLAKE2S_BLOCKBYTES);
